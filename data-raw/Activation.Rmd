...
# Spillover Matrix
spill <- read.csv(paste0(DataPackageR::project_extdata_path(),"/","Compensation-Spillover-Matrix.csv"), header = TRUE, row.names = 1)
colnames(spill) <- rownames(spill)
spill <- as.matrix(spill)
rownames(spill) <- NULL
lapply(seq_along(Activation), function(x){
  Activation[[x]]@description$SPILL <<- spill
})
```
